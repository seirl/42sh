              ...
              int len)
{
    ...
    for (int i = 0; i < len; ++i)
    {
        ...
        if (pid == 0)
        {
            ...
            exec_cmd_node(shell, pipe_cmds[i]);
            if (shell->status < 0)
...
static void wait_pipe(s_shell *shell, pid_t main, s_pipe *pipe)
{
    int st = 0;
    pid_t proc = 0;
...
void exec_pipe_node(s_shell *shell, s_ast_pipeline *node)
{
    ...
    if (len == 1)
        exec_cmd_node(shell, pipe_cmds[0]);
    else if (len > 1)
    {
        s_pipe *pipe = pipe_init();
        int res = exec_pipe(shell, pipe, pipe_cmds, len);
        if (res != 0)
        {
